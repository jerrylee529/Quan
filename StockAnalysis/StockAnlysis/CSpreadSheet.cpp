...
SpreadSheet::SpreadSheet(CString File, CString SheetOrSeparator, bool Backup) :
		...
		if (Open())
		{
			if (m_bBackup)
			{
				if ((m_bBackup) && (m_bAppend))
				{
					...
					m_sSheetName = "SpreadSheetBackup";
					m_bAppend = false;
					if (!Commit())
					{
						m_bBackup = false;
...
bool SpreadSheet::AddHeaders(CStringArray &FieldNames, bool replace)
{
...
bool SpreadSheet::DeleteSheet(CString SheetName)
{
	if (m_bExcel) // If file is an Excel spreadsheet
	{
		// Delete sheet
...
bool SpreadSheet::AddRow(CStringArray &RowValues, long row, bool replace)
{
	long tempRow;
...
bool SpreadSheet::AddCell(CString CellValue, CString column, long row, bool Auto)
{
...
// Default is add cell into new row.
bool SpreadSheet::AddCell(CString CellValue, short column, long row)
{
	if (column == 0)
	{
...
bool SpreadSheet::ReplaceRows(CStringArray &NewRowValues, CStringArray &OldRowValues)
{
	if (m_bExcel) // If file is an Excel spreadsheet
	{
		m_Database->OpenEx(m_sDsn, CDatabase::noOdbcDialog);
...
bool SpreadSheet::ReadRow(CStringArray &RowValues, long row)
{
...
// Set Auto to false if want to force column to be used as header name
bool SpreadSheet::ReadColumn(CStringArray &ColumnValues, CString column, bool Auto)
{
	short columnIndex = CalculateColumnNumber(column, Auto);
	if (columnIndex == 0)
	{
...
bool SpreadSheet::ReadColumn(CStringArray &ColumnValues, short column)
{
	if (column == 0)
	{
		m_sLastError = "Column cannot be zero\n";
...
// Set Auto to false if want to force column to be used as header name
bool SpreadSheet::ReadCell (CString &CellValue, CString column, long row, bool Auto)
{
	short columnIndex = CalculateColumnNumber(column, Auto);
	if (columnIndex == 0)
	{
...
// Default is read the next cell in next row.
bool SpreadSheet::ReadCell (CString &CellValue, short column, long row)
{
	if (column == 0)
	{
		m_sLastError = "Column cannot be zero\n";
...
// Begin transaction
void SpreadSheet::BeginTransaction()
{
	m_bTransaction = true;
}

// Save changes to spreadsheet
bool SpreadSheet::Commit()
{
	if (m_bExcel) // If file is an Excel spreadsheet
	{
		m_Database->OpenEx(m_sDsn, CDatabase::noOdbcDialog);
...
bool SpreadSheet::Convert(CString SheetOrSeparator)
{
	// Prepare file
	m_stempString = m_sFile;
	...
	{
		m_stempString += ".csv";
		SpreadSheet tempSheet(m_stempString, SheetOrSeparator, false);
...
short SpreadSheet::CalculateColumnNumber(CString column, bool Auto)
{
	if (Auto)
	{
...
void SpreadSheet::GetExcelDriver()
{
	char szBuf[2001];
	WORD cbBufMax = 2000;
	WORD cbBufOut;
